...
import { decryptToken, encryptToken } from "@wezesha/db/crypto";
import {
  QuickBooksAuthError,
  refreshAccessToken,
  revokeToken,
 ...
 *
 * It lives in the package rather than in apps/web because the worker needs it
 * too, and the worker cannot import from apps/web. Intuit invalidates a refresh
 * token the moment it issues the next one, so two copies of this rotating
 * independently would leave whichever ran second holding a dead grant — which
 * surfaces as the customer appearing to have revoked us.
 */
