...
| `AUDIT_ENGINE_URL` | `packages/forecast-run/src/engine-client.ts` (via web `/api/forecast/onboarding-audit`) | web | Vercel (+ Railway if the worker ever triggers it) | url (config) | unset — the onboarding audit reports `engine_unreachable` and no-ops, so the app runs fine without it; set to the deployed `inventory-audit-engine` base URL to enable the button |
