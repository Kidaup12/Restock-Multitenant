...
export async function getUnreadCount(tenantId: string): Promise<number> {
  return prismaForTenant(tenantId).notification.count({ where: { readAt: null } });
}

/** Newest-first page of the tenant's feed, cursor-paginated by row id. */
export async function listNotifications(
  tenantId: string,
  { cursor, limit = DEFAULT_PAGE_SIZE }: { cursor?: string | null; limit?: number } = {}
